<?php 
	 ...
	 */
	 ...
	 /*** CSS DO TEMA ***/
	 echo $this->Html->css(array('bootstrap.min','bootstrap-responsive.min','/js/plupload/jquery.plupload.queue/css/jquery.plupload.queue','ger-imagem'),null,array('inline'=>false));
	 
	 /*** JS DO TEMA ***/
	 echo $this->Html->script(array('http://bp.yahooapis.com/2.4.21/browserplus-min.js','tmp/jquery.min.js','plupload/plupload.full','plupload/jquery.plupload.queue/jquery.plupload.queue','bootstrap.min'),array('inline'=>false));
...
<html>
  <head>
    ...
    <?php echo $this->element('seo',array('canonical'=>null,'keywords'=>null,'description'=>null,'robots'=>null));?>
	<?php
...
<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  if($('.plupload').length > 0){
	...
	$('.plupload').pluploadQueue({
		...
		// Flash settings
		flash_swf_url : '<?php echo $this->Html->url('/js');?>/plupload/plupload.flash.swf',

		// Silverlight settings
		silverlight_xap_url : '<?php echo $this->Html->url('/js');?>/plupload/plupload.silverlight.xap',
	  ...
	  function seleciona(url){
		...
		$('#myModal .tamanho').click(function(){
			...
			<?php elseif(isset($_GET['elementid'])):?>
				...
				window.opener.$('.arquivo').val(url);
				window.opener.$('#<?php echo $_GET['elementid'];?>').addClass('btn-success');
				window.close();
			<?php endif;?>
			return false;
		});
